...
export default function Home() {
  ...
  const { data: election_state } = useReadContract({
    abi: VOTE_ABI,
    functionName: 'get_election_state',
    address: VOTE_CONTRACT_ADDRESS,
    args: [],
  ...
  const { data: election_id } = useReadContract({
    abi: VOTE_ABI,
    functionName: 'get_election_id',
    address: VOTE_CONTRACT_ADDRESS,
    args: [],
    refetchInterval: 10000,
  });

  //has_voted takes just voter_address — the contract resolves election_id internally
  const { data: hasVoted } = useReadContract({
    abi: VOTE_ABI,
    functionName: 'has_voted',
    address: VOTE_CONTRACT_ADDRESS,
    ...
    refetchInterval: 10000,
    enabled: !!address && Number(election_state) === 1,
  });

  useEffect(() => {
    setElection_s({
      candidates_length: candidates_length as bigint,
      election_state: election_state as bigint,
      calculate_votes: calculate_votes as { 0: bigint; 1: bigint }[],
      election_id: election_id as bigint,
    });
  }, [candidates_length, election_state, calculate_votes, election_id]);

  const { send: sendVote } = useSendTransaction({});
  const { send: sendReset } = useSendTransaction({});
  ...
  const handleVote = (candidateId: bigint) => {
    ...
    if (Number(election_state) !== 1) return;

    try {
      sendVote([contract.populate('vote', [candidateId])]);
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        {Number(election_s?.election_state) === 1 && (
          <div className="mb-8 animate-slide-up">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {get_all_candidates(election_s)?.map((candidate) => (
                <div key={candidate[0].toString()} className="animate-slide-up">
                  <CandidateCard
                    candidate={candidate}
                    mockCandidate={mockCandidate}
                    onVote={handleVote}
